...
build-docker-prod:
	docker build -t mattgleich/clockhat:latest .
build-docker-dev:
	docker build -f dev.Dockerfile -t mattgleich/clockhat:test .
build-docker-dev-lint:
	docker build -f dev.lint.Dockerfile -t mattgleich/clockhat:lint .
build-go:
	go get -v -t -d ./...
	go build -v .
	rm clockhat
